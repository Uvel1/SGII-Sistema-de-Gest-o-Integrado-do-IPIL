...
export function LoginAlunoForm({
  ...
  const validateForm = () => {
    ...
    if (!isValid) {
      toast.error(errorMessage); // Exibe os erros em um alert
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      try {
        const response = await axios.post<{ accessToken: string; refreshToken: string; userType:{ tipo: string }[] }>("http://localhost:3333/Entrar_alunos", formData);
        ...
        const userTypeValue = decoded.tipo;
        localStorage.setItem("userType", userTypeValue);

        console.log(userType,accessToken,refreshToken);
        router.push("/alunos");
      } catch (error) {
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-6">
        ...
        <Button type="button" onClick={() => router.push('/auth/verificarConta')} variant="outline" className="w-full text-blue-700 hover:text-blue-700">
          Verificar Conta
        </Button>
      </div>
    </form>
