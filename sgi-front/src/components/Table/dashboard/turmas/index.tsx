...
import { EditarTurma } from "@/components/Dialog/editar/turma";
import { ApagarTurma } from "@/components/Alerts/apagar/turma";
...
export function TableTurma() {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:3333/tabela_turmas_dashboard`);
    ...
    React.useEffect(() => {
      const socket = new WebSocket("ws://localhost:3333/tabela_turmas_dashboard");
  ...
  return (
    <>
      <div className="flex flex-col md:flex-row md:items-center justify-between py-4 md:space-x-4">
        ...
        <Button className="bg-blue-700 font-bold hover:bg-blue-800 w-32">Adicionar Turma</Button>
      </div>
