...
interface ApagarPedidoRecebidoProps {
  id: number; // ou string, conforme o identificador utilizado
}

export function ApagarPedidoRecebido({ id }: ApagarPedidoRecebidoProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`http://localhost:3333/pedidos/${id}`);
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle className="text-red-600">
            Tem certeza absoluta que quer apagar?
          </AlertDialogTitle>
          <AlertDialogDescription>
            Esta ação vai apagar o arquivo permanentemente.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
